  ...
  DS.LSAdapter = DS.Adapter.extend(Ember.Evented, {
    ...
    find: function(store, type, id, opts) {
      ...
      if (!record || !record.hasOwnProperty('id')) {
        return Ember.RSVP.reject(new Error("Couldn't find record of" + " type '" + type.typeKey + "' for the id '" + id + "'."));
      }
    ...
    findMany: function(store, type, ids, opts) {
      ...
      for (var i = 0; i < ids.length; i++) {
        ...
        if (!record || !record.hasOwnProperty('id')) {
          return Ember.RSVP.reject(new Error("Couldn't find record of type '" + type.typeKey + "' for the id '" + ids[i] + "'."));
        }
    ...
    createRecord: function(store, type, record) {
      ...
      var recordHash = record.serialize({
        includeId: true
      });
    ...
    updateRecord: function(store, type, record) {
      ...
      namespaceRecords.records[id] = record.serialize({
        includeId: true
      });
